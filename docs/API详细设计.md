...
管理端详情根据 `base_url` 和模型类型展示完整调用 URL、用途、运行参数、状态及凭据版本；API Key 仅显示
“已加密配置/未配置”，不得提供查看、复制或下载明文的能力。详情同时生成按模型类型区分的 curl 验证模板，
认证值必须使用 `${QIANFAN_API_KEY}` 环境变量占位符，不得将数据库凭据写入命令。连通性测试期间管理端展示
进行中进度，完成后展示平台请求标识或脱敏错误及重试入口。

生产完整 UAT 必须分别对 `deepseek-v4-pro`、`ernie-5.0`、`embedding-v1` 和 `bce-reranker-base` 调用上述
连通性接口，并通过模型调用日志将平台请求标识与 `chat`、`multimodal_chat`、`embedding`、`rerank` 成功记录
逐项匹配。任一活动配置缺失、模型编码不符、请求失败或审计记录缺失均阻断签署。
